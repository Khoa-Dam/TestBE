  ...
  async randomMint(
    ...
    try {
      ...
      try {
        ...
        console.debug(
          "workflow-new.randomMint: collectionTypeField, hasManifest, isNonManifest ->",
          { collectionTypeField, hasManifest: !!draft.manifest, isNonManifest }
        );
        if (isNonManifest) {
          data = await mintNonManifest(this.draftId, payerAddr, toAddr);
        } else {
...
function getMockCollections(limit = 50, offset = 0): CollectionsResponse {
  const mockCollections: Collection[] = [
    ...
    },
    {
      ...
      creator_address:
        "0x999888777666555444333222111000999888777666555444333222111000999888",
      max_supply: 800,
...
function getMockNFTs(collectionId: string): NFTsResponse {
  console.log(
    "getMockNFTs: Generating mock NFTs for collection:",
    collectionId
  );
  ...
  const mockNFTs: NFT[] = Array.from(
    { length: Math.floor(Math.random() * 20) + 5 },
    (_, i) => {
      const attributes = [
        {
          trait_type: "Background",
          value: ["Blue", "Red", "Green", "Purple", "Yellow"][
      ...
      if (Math.random() > 0.5) {
        attributes.push({
          trait_type: "Type",
          value: ["Digital Art", "Collectible", "Gaming", "Utility"][
      ...
      const nft: NFT = {
        token_data_id: `${collectionId.slice(0, 20)}${i
          .toString()
          .padStart(3, "0")}`,
        ...
        image_uri: `https://picsum.photos/400/400?random=${collectionId.slice(
          -4
        )}${i}`,
        attributes,

...
export const Mint: React.FC<MintProps> = ({ draftId, onBack }) => {
  ...
  const handleAddAllowlist = async () => {
    ...
    try {
      ...
      const txHash = await workflow.addAllowlist(
        addrs,
        signer,
        account.address
      );
  ...
  const handleMint = async (phase: "presale" | "public") => {
    ...
    if (!isMintingAllowed(phase)) {
      alert(
        `${
          phase === "presale" ? "Presale" : "Public"
        } minting is not currently available`
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `🚀 Starting ${phase} mint for collection:`,
        collectionDetails._id
      );
      console.log("Payer address:", account.address);

      // Step 1: Call appropriate mint API to get transaction
      let mintResult: any;
      // Robust detection: backend may provide collection_type in several places
      const cdAny = collectionDetails as any;
      const collectionTypeField =
        cdAny.collection_type ||
        cdAny.collectionType ||
        cdAny.collection_type ||
        cdAny.config?.collection_type ||
        cdAny.config?.collectionType ||
        cdAny.collection?.type ||
        cdAny.collection?.collection_type ||
        cdAny.type ||
        cdAny.kind;
      const isNonManifest =
        collectionTypeField === "non-manifest" ||
        collectionTypeField === "nonManifest" ||
        !collectionDetails.manifest;
      console.debug(
        "Mint: collectionTypeField, hasManifest, isNonManifest ->",
        {
          collectionTypeField,
          hasManifest: !!collectionDetails.manifest,
          isNonManifest,
          collectionDetails,
        }
      );
      if (isNonManifest) {
        // Non-manifest / manual supply collection
        mintResult = await mintNonManifest(
          collectionDetails._id,
          account.address,
          account.address,
        ...
        // Manifest-based random mint
        try {
          mintResult = await randomMint(
            collectionDetails._id,
            account.address,
            account.address // toAddr same as payerAddr
          );
        } catch (err: any) {
          // If backend tells us to use mint-non-manifest, fallback automatically
          const msg =
            typeof err === "string" ? err : err?.message || err?.toString();
          console.warn("randomMint failed:", msg);
          if (msg && msg.includes("mint-non-manifest")) {
            console.debug(
              "Falling back to mintNonManifest due to server response"
            );
            mintResult = await mintNonManifest(
              collectionDetails._id,
              account.address,
      ...
      try {
        ...
        const txResult: any = await aptos.waitForTransaction({
      ...
      try {
        ...
        console.warn(
          "⚠️ Failed to mark as minted, but transaction was successful:",
          syncError
        );
      ...
      const newMintedTokens = [
        ...(collectionDetails.mintedTokenIndexes || []),
        mintResult.tokenIndex,
      ];
      ...
      const price =
        phase === "presale"
          ? (
              parseInt(collectionDetails.config?.pricing?.presale || "0") /
              Math.pow(10, 8)
            ).toFixed(2)
          : (
              parseInt(collectionDetails.config?.pricing?.public || "0") /
              Math.pow(10, 8)
            ).toFixed(2);
      ...
      const nftName =
        mintResult.metadata?.name || `NFT #${mintResult.tokenIndex}`;
      alert(
        `🎉 Successfully minted "${nftName}" from ${phase} phase!\n\n` +
          `Price: ${price} ${
            collectionDetails.config?.pricing?.currency || "APT"
          }\n` +
  ...
  const isMintingAllowed = (phase: "presale" | "public") => {
    ...
    const { setPhaseManual, phaseManual, freezeAfter, schedule } =
    ...
    if (freezeAfter && schedule) {
      const now = Date.now();
      const presaleStartMs = schedule.presaleStart
        ? schedule.presaleStart * 1000
        : 0;
      const publicStartMs = schedule.publicStart
        ? schedule.publicStart * 1000
        : 0;
      const saleEndMsRaw = schedule.saleEnd ? schedule.saleEnd * 1000 : 0;
      // If saleEnd is missing or <= publicStart, treat as no end (Infinity)
      const saleEndMs =
        !saleEndMsRaw || saleEndMsRaw <= publicStartMs
          ? Infinity
          : saleEndMsRaw;
  ...
  const getAvailableNFTs = () => {
    ...
    return collectionDetails.manifest.filter(
      (_, index) => !collectionDetails.mintedTokenIndexes?.includes(index)
    );
  };
  ...
  const getCountdownTime = (phase: "presale" | "public") => {
    ...
    console.log(
      "🔍 collectionDetails?.config?.schedule:",
      collectionDetails?.config?.schedule
    );
    ...
    if (phase === "presale") {
      ...
      if (publicStartMs && now >= presaleStartMs && now < publicStartMs)
        return publicStartMs;
    } else {
      ...
      if (saleEndMs && now >= publicStartMs && now < saleEndMs)
        return saleEndMs;
    }
      ...
      >
        ...
        >
          ...
          <div style={{ marginBottom: "24px" }}>
            ...
            {Boolean(
              collectionDetails.config?.pricing?.presale &&
                collectionDetails.config?.pricing?.presale !== "0"
            ) && (
              <div
                style={{
                  marginBottom: "16px",
              ...
              >
                ...
                >
                  <span>
                    ⚠️ Presale yêu cầu allowlist. Hãy thêm allowlist trước khi
                    mở presale.
                  </span>
                  <button
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setUseDatabaseAllowlist(e.target.checked)
                        }
                      />
                      <span>
                        Sử dụng danh sách allowlist đã lưu trong database
                      </span>
                    </label>
            ...
            >
              {/* Custom metadata for non-manifest collections */}
              {!collectionDetails?.manifest && (
                <div
                  style={{
                    padding: "12px",
                ...
                >
                  <h4 style={{ margin: "0 0 8px 0" }}>
                    Optional: Custom metadata for this mint
                  </h4>
                  <input
                    placeholder="Custom name (e.g. My NFT #1)"
                    value={customName}
                  ...
                  <div style={{ fontSize: 12, color: "#6c757d", marginTop: 8 }}>
                    These values will be used for the minted token's metadata
                    when this collection does not use a manifest.
                  </div>
                </div>
              )}
              {/* Presale Phase */}
          ...
          >
            ...
            <div style={{ marginBottom: "4px" }}>
              • Per Wallet Cap:{" "}
              <span style={{ color: "#212529", fontWeight: "500" }}>
                {collectionDetails.config?.supply?.perWalletCap || "N/A"}
              </span>
            </div>
            <div>
              • Total Supply:{" "}
              <span style={{ color: "#212529", fontWeight: "500" }}>
                {collectionDetails.config?.supply?.max || "N/A"}
              </span>
            </div>
          </div>
        </div>
